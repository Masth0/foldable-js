...
export default {
  setup() {
    let foldables = ref([]);
    let navburgerFoldable = ref(null);

    const onClickOutsideChange = function(value) {
      if (foldables.value.length > 0) {
        for (let i = 0; i <foldables.value.length; i++) {
          foldables.value[i].clickOutside = value;
        }
      }
      if (navburgerFoldable !== null) {
        navburgerFoldable.value.clickOutside = value;
      }
    }
    ...
    const onFocusTrapChange = function(value) {
      if (foldables.value.length > 0) {
        for (let i = 0; i <foldables.value.length; i++) {
          foldables.value[i].focusTrap = value;
        }
      }
      if (navburgerFoldable !== null) {
        navburgerFoldable.value.focusTrap = value;
      }
    }
